...
- For Superpowers and multi-agent Task dispatches in Cursor, default to Grok 4.6 extra high (Fast Mode). Prefer Kimi K3, GLM 5.2, and other cheap enabled models when a different cheap model is needed. Elevate to Sonnet 5 or Opus 5 when planning or orchestration decides a stronger model is needed, or when a cheaper subagent is stuck / cannot finish. Use Opus 5 only as a last resort after Sonnet 5 is stuck — it is expensive. Always pass `model` explicitly; never inherit. GPT 5.6 is allowed if enabled but is not a default. Pick the specialized agent best suited to each task.
- When running in Antigravity (Antigravity IDE or Antigravity CLI `agy`), use **only Gemini 3.7** (`high`, `medium`, or `low` thinking effort) to do anything. During planning and orchestration, decide what Gemini 3.7 tier to use for each task (Low for simple mechanical/lint/script tasks, Medium for standard features/components/tests/reviews, High for architecture/planning/multi-crate/IPC/security/complex debugging). Tasks in Antigravity are labeled `- [ ] Task N: Title (Gemini 3.7 High/Medium/Low) — outcome clause`. If any task or subagent gets stuck on a lower tier, always escalate directly to Gemini 3.7 High; never switch outside Gemini 3.7.
...
- Always use Superpowers SDD (specs, plans, tasks, reviews) when using Superpowers. Keep all Superpowers files under `.superpowers/`: specs in `.superpowers/docs/specs/`, plans in `.superpowers/docs/plans/`, tasks in `.superpowers/docs/tasks/` (same slug as the plan, listing associated `Spec:` + `Plan:` links and per-task model assignments; keep tasks list simple with `- [x]` high-level outcome clauses), reviews in `.superpowers/docs/reviews/`. Do not write `docs/superpowers/`, `docs/plans/`, or `docs/specs/`. Track `.superpowers/docs/`; gitignore only `.superpowers/brainstorm/` and `.superpowers/sdd/`. Maintain `.superpowers/sdd/<plan-slug>/progress.md` (or `.superpowers/sdd/progress.md`) as the detailed live SDD execution ledger: track the active task in real time (model, current stage, target files, next action) and record a concise 2-3 line summary per completed task (changes, files, tests, commits, review verdict). Always generate and maintain the full suite of SDD companion artifacts alongside `progress.md` (`task-N-brief.md`, `task-N-report.md`, `review-task-N.diff`, `task-N-review.md`, whole-branch reviews). When implementing an approved spec, use Superpowers subagent-driven development with TDD unless asked otherwise. Terminal pane work is not done until a real WebView2 visual check (Playwright/CDP) shows a cursor and prompt; jsdom and `http://localhost:5173` have no Tauri IPC and do not count.
- Shell and chrome UI text is not selectable; keep the content area copyable. The statusbar has no right-click context menu. Keep Files-tab toolbar actions, side-panel Files/Agent/Settings tabs, and settings/config rows compact; oversized chrome looks unprofessional.
...
- Apps follow OS appearance by default: dark → Polar Night, light → Snow Storm; Polar Night if theme IPC fails or returns null. Both Terminal and Explorer also expose an explicit override (Polar Night / Snow Storm / Match system) in their Config tab's Appearance sub-tab.
...
- `gencore-pty` IPC arguments are snake_case (`session_id`, not `sessionId`); a camelCase mismatch drops CPR/write/resize and leaves a blank pane. The Isolation hook (`apps/terminal/isolation/isolation.hook.js`) is a strict key allowlist: new `open` fields such as `posh_theme` must be validated and reconstructed there, or `open` is rejected and the pane can show replayed prompt text while swallowing all typing. `scripts/tauri-dev-terminal.mjs` launches Terminal `tauri:dev` with WebView2 `--remote-debugging-port=9223` and deletes `CURSOR_AGENT` (Oh My Posh `init` prints nothing when that env is set). Playwright visual tests attach to that WebView2, never to a normal `http://localhost:5173` tab.
- PNG/ICO and other binaries listed in `.gitattributes` are Git LFS. Husky owns `core.hooksPath`, so Git's built-in LFS hooks do not run; `.husky/pre-push` must run `git lfs` and also `git lfs push … HEAD` because GUI clients often invoke the hook with empty stdin (GitHub then rejects with `GH008`). Do not skip hooks (`--no-verify` / `HUSKY=0`) — that skips the LFS upload. Pre-push is LFS-only; lint/test stay in CI. Keep `.husky/**` LF.
- `changeset-release/main` is the Changesets GitHub Action working branch and may reappear on `main` pushes that have pending changesets; it is not leftover feature work.
