...
- Keep all Superpowers files under `.superpowers/`: specs in `.superpowers/docs/specs/`, plans in `.superpowers/docs/plans/`, tasks in `.superpowers/docs/tasks/` (same slug as the plan, listing associated `Spec:` + `Plan:` links and per-task model assignments; keep tasks list simple with `- [x]` high-level outcome clauses), reviews in `.superpowers/docs/reviews/`. Do not write `docs/superpowers/`, `docs/plans/`, or `docs/specs/`. Track `.superpowers/docs/`; gitignore only `.superpowers/brainstorm/` and `.superpowers/sdd/`. Maintain `.superpowers/sdd/<plan-slug>/progress.md` (or `.superpowers/sdd/progress.md`) as the detailed live SDD execution ledger: track the active task in real time (model, current stage, target files, next action) and record a concise 2-3 line summary per completed task (changes, files, tests, commits, review verdict). Always generate and maintain the full suite of SDD companion artifacts alongside `progress.md` (`task-N-brief.md`, `task-N-report.md`, `review-task-N.diff`, `task-N-review.md`, whole-branch reviews).
