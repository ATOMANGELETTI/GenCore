...
- Official Nord hex only (`nord0`–`nord15`). Polar Night is the fallback default (dark); apps drive OS appearance sync. Light theme is Snow Storm. Frost is primary/ring/selected text (`nord8` dark, `nord10` light selected text); Aurora is destructive/caution/warning/success/info (`nord11`–`nord15`)
