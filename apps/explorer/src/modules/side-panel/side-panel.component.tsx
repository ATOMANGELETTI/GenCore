...
import { FolderTree as FolderTreeIcon, Info, type LucideIcon, Settings } from "lucide-react";
...
export function SidePanel({ open = true, currentPath, selectedPaths, onNavigate }: SidePanelProps) {
  const [selected, setSelected] = React.useState<SidePanelTabId>("tree");
        ...
        {TABS.map((tab) => (
          ...
          >
            {tab.id === "tree" ? (
              ...
              <Details selectedPaths={selectedPaths} />
            ) : tab.id === "config" ? (
              <Config />
            ) : null}
          </div>
