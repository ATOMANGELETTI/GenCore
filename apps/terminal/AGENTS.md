...
- Capabilities grant the matching `gencore-fs:allow-*`, `gencore-pty:allow-open` / `allow-write` / `allow-resize` / `allow-close`, `gencore-core:allow-get-system-telemetry`, `gencore-core:allow-load-pinned-tabs` / `allow-save-pinned-tabs`, plus `core:window:allow-theme` and `core:event:allow-listen` / `allow-unlisten`
