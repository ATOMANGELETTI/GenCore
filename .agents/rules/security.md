...
- Capabilities (`src-tauri/capabilities/*.json`): the **main** capability stays
  `"windows": ["main"]` only; any extra window (for example `tray-menu`) gets its
  **own** capability file scoped to that label, still least privilege, never
  `core:default`. Grant only the identifiers the UI invokes
  (`core:window:allow-close`, `gencore-core:allow-get-app-info`, …) — never
  `core:default`.
