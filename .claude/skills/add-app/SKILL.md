...
4. `tauri.conf.json`:
   ...
   - `app.security.csp` object form (see the security baseline in the root `AGENTS.md`);
...
7. Add the app's dev port to `.cursor/environment.json` `ports` if it needs cloud
   preview (coordinate with the controller — this file is shared), and to
   `.claude/launch.json` if it should be previewable from Claude Code.
8. Verify: `pnpm --filter @gencore/{name} typecheck`, `test`, `lint`;
   `cargo check -p gencore-{name}`.

## Constraints

Follow the architecture, security, naming, and UI-kit conventions in the root `AGENTS.md`.
